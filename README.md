![](https://img.shields.io/badge/Learning%20Progress-Day2-blue)

# 👋 Hi, I'm Zoe / 你好，我是金志英

> **Accounting Graduate | Learning Finance Automation**  
> **会计专业毕业生 | 正在学习财务自动化**

---

## 📚 About Me / 关于我

- 🎓 **Background** / 教育背景  
  Accounting from Nanchang Hangkong University / 南昌航空大学 会计专业

- 📊 **Currently Learning** / 正在学习  
  1. Excel Automation: Power Query, PivotTables / Excel 自动化：Power Query、数据透视表  
  2. Python for Data Analysis: openpyxl, Pandas / Python 数据分析  
  3. GitHub & Version Control / GitHub 版本控制

- 🌱 **Goal** / 我的目标  
  Become a **tech-savvy finance professional** / 成为一名懂技术的复合型财务人才

---

## 📅 Learning Log / 学习日志

- [x] Day 1: GitHub Registration & Personal Homepage / 注册 GitHub，创建个人主页  
- [x] Day 2: Markdown Basics & Profile Polish / 学习 Markdown 语法，美化主页  
- [x] Day 3：Excel 收支表 + Python 第一个程序Hello + MySQL 安装  
- [x] Day 4：Excel 产品毛利分析 + Python 分析脚本尝试 + SQL 基础查询入门
- [x] Day 5：Excel 数据透视表练习 + Python 清洗脚本运行成功 + SQL 基础复习
- [x] Day 6：Excel VLOOKUP 进阶 + pandas 基础操作 + SQL 多表连接/子查询
- [x] Day 8：Excel Power Query 合并多文件 + pandas 进阶（分组聚合、透视表、多表合并）+ SQL 窗口函数（ROW_NUMBER、RANK、LAG、累计求和）
- [x] Day 9：pandas数据处理进阶（缺失值/重复值/异常值处理、标准化）+ Power Query进阶（合并查询、分组透视）+ SQL窗口函数进阶（分组排名、累计、环比、帕累托分析）
- [x] Day 11：Excel Power Query 进阶（合并查询 + 透视列）+ Python 数据可视化入门（Matplotlib/Seaborn）+ SQL 复杂查询优化（CTE、同比环比、移动平均、索引）
- [x] Day 12：Excel 动态财务仪表板制作（数据透视表 + 切片器 + 图表联动）+ Python 金蝶报表综合财务分析（收入趋势、费用结构、利润柱状图、产品毛利对比）+ SQL 财务高级分析（CASE 条件聚合、累计值、占比计算、临时表、查询优化）
---



## 📫 Contact / 联系我

- Email / 邮箱:3077656384@qq.com
- GitHub: 08Zoe18
- LinkedIn / 领英: [你的领英链接]（可选）

---
